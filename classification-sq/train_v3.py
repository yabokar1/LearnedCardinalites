...
from util_v3 import *
from data_v3 import get_train_datasets, load_data, make_dataset
from model_v3 import SetConv
...
def predict(model, data_loader):
    ...
    for batch_idx, data_batch in enumerate(data_loader):
        ...
        '''for i in range(outputs.data.shape[0]):
            _, predicted = torch.max(outputs.data[i], 1)
            preds.append(predicted)'''

    # values = [tensor.numpy()[0] for tensor in preds]
    # values = [tensor.item() for tensor in preds]
