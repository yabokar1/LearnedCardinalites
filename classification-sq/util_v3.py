...
# SQ: creates a list of unique column names from the training set of queries
...
def get_all_table_names(tables):
    ...
    for query in tables:
        for table in query:
            # SQ: added to skip empty string in the table list
...
# SQ: the following function takes as input a set of string
# and returns 2 output: (1) an 1-hot encoding of these string values (e.g., table names)
# (2) a list of these string values
...
def normalize_data(val, column_name, column_min_max_vals):
    ...
    # SQ: commeted out the following line
    # val = float(val)
    # SQ: added below code block - if and else block
    if isinstance(val, str):
        val = (hash(val)% 1000)
        # print(f"The val is {val}")
    else:
        val = float(val)
    # SQ: adding code ended
    val_norm = 0.0
    if max_val > min_val:
...
def encode_samples(tables, samples, table2vec):
    ...
    for i, query in enumerate(tables):
        ...
        for j, table in enumerate(query):
            ...
            # print(f"The table vector is {table2vec}")
            # print(f"The table is {table}")
            # SQ: strip() the table name in cases where the table name has extra space, e.g. ' inventory'
            sample_vec.append(table2vec[table.strip()])
            # Append bit vector
...
# SQ: encode the value of the predicates and the joins for each query
def encode_data(predicates, joins, column_min_max_vals, column2vec, op2vec, join2vec):
    ...
    for i, query in enumerate(predicates):
        ...
        for predicate in joins[i]:
            # Join instruction
            #SQ: added strip() to remove any whitespace such as in ' i_item_sk=cs_item_sk'
            join_vec = join2vec[predicate.strip()]
